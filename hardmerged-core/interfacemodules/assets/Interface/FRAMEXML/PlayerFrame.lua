...
--		end
	...
	elseif ( event == "UNIT_DARK_MANA" and arg1 == "player" ) then
		PlayerFrame_SetDarkMana(UnitPower("player"));
...
function PlayerFrame_SetupDeathKnniggetLayout ()
	...
	PlayerFrame:RegisterEvent("UNIT_DARK_MANA");
	...
	PlayerFrame:CreateTexture("PlayerFrameDarkManaOverlay", "OVERLAY");
	PlayerFrameDarkManaOverlay:SetPoint("TOPLEFT", 15, 15);
	PlayerFrameDarkManaOverlay:SetWidth(256);
	PlayerFrameDarkManaOverlay:SetHeight(128);
	PlayerFrameDarkManaOverlay:SetTexture("Interface\\PlayerFrame\\UI-PlayerFrame-DeathKnight-GoldBorder");

	-- Death Knight Dark Mana Glow
	local frame = CreateFrame("Frame");
	frame:SetPoint("TOPLEFT", PlayerFrame);
	frame:SetPoint("BOTTOMRIGHT", PlayerFrame);

	frame:CreateTexture("PlayerFrameDarkManaGlow", "BORDER");
	PlayerFrameDarkManaGlow:SetPoint("TOPLEFT", 15, 15);
	PlayerFrameDarkManaGlow:SetPoint("BOTTOMRIGHT", PlayerFrameBackgroundTexture);
	PlayerFrameDarkManaGlow:SetAlpha(0.050);
	PlayerFrameDarkManaGlow:SetWidth(256);
	PlayerFrameDarkManaGlow:SetHeight(128);
	PlayerFrameDarkManaGlow:Hide();
	PlayerFrameDarkManaGlow:SetTexture("Interface\\PlayerFrame\\UI-PlayerFrame-DeathKnight-Sword-Glow");
	...
	local darkManaBar = PlayerFrame:CreateTexture("$parentDarkManaBar", "ARTWORK");
	-- Mmmm, dark Mana Bars, my favorite kind. Tastes like Death Knnigget.
	darkManaBar:SetTexture("Interface\\PlayerFrame\\UI-PlayerFrame-DeathKnight-DarkMana");
	darkManaBar:SetPoint("BOTTOMLEFT", "$parent", "BOTTOMLEFT", 70, 24);
	darkManaBar:SetWidth(64);
	darkManaBar:SetHeight(63);
...
local DARKMANABARHEIGHT = 63;
local DARKMANABARWIDTH = 64;

local DARKMANAGLOW_FADEALPHA = .050
local DARKMANAGLOW_MINALPHA = .40
local DARKMANAGLOW_MAXALPHA = .80
local DARKMANAGLOW_THROBINTERVAL = .8;

DARKMANAGLOW_FINISHTHROBANDHIDE = false;
local DARKMANAGLOW_THROBSTART = 0;

function PlayerFrame_SetDarkMana (darkMana)
	PlayerFrameDarkManaBar:SetHeight(DARKMANABARHEIGHT * (darkMana / 100));
	PlayerFrameDarkManaBar:SetTexCoord(0, 1, (1 - (darkMana / 100)), 1);
	
	if ( darkMana >= 90 ) then
		-- Oh,  God help us for these function and variable names.
		DARKMANAGLOW_FINISHTHROBANDHIDE = false;
		if ( not PlayerFrameDarkManaGlow:IsShown() ) then
			PlayerFrameDarkManaGlow:Show();
		end
		PlayerFrameDarkManaGlow:GetParent():SetScript("OnUpdate", DeathKnniggetThrobFunction);
	elseif ( PlayerFrameDarkManaGlow:GetParent():GetScript("OnUpdate") ) then
		DARKMANAGLOW_FINISHTHROBANDHIDE = true;
	else
		PlayerFrameDarkManaGlow:Hide();
	end
end
...
function DeathKnniggetThrobFunction (self, elapsed)
	if ( DARKMANAGLOW_THROBSTART == 0 ) then
		DARKMANAGLOW_THROBSTART = GetTime();
	elseif ( not DARKMANAGLOW_FINISHTHROBANDHIDE ) then
		local interval = DARKMANAGLOW_THROBINTERVAL - math.abs( .9 - (UnitPower("player") / 100)); 
		local animTime = GetTime() - DARKMANAGLOW_THROBSTART;
		if ( animTime >= interval ) then
			-- Fading out
			PlayerFrameDarkManaGlow:SetAlpha(math.max(DARKMANAGLOW_MINALPHA, math.min(DARKMANAGLOW_MAXALPHA, DARKMANAGLOW_MAXALPHA * interval/animTime)));			
			...
			-- Fading in
			if ( firstFadeIn ) then
				PlayerFrameDarkManaGlow:SetAlpha(math.max(DARKMANAGLOW_FADEALPHA, math.min(DARKMANAGLOW_MAXALPHA, DARKMANAGLOW_MAXALPHA * animTime/interval)));			
			else
				PlayerFrameDarkManaGlow:SetAlpha(math.max(DARKMANAGLOW_MINALPHA, math.min(DARKMANAGLOW_MAXALPHA, DARKMANAGLOW_MAXALPHA * animTime/interval)));			
			end
		end
	elseif ( DARKMANAGLOW_FINISHTHROBANDHIDE ) then
		local currentAlpha = PlayerFrameDarkManaGlow:GetAlpha();
		local animTime = GetTime() - DARKMANAGLOW_THROBSTART;
		local interval = DARKMANAGLOW_THROBINTERVAL;
		firstFadeIn = true;
		
		if ( animTime >= interval ) then
			-- Already fading out, just keep fading out.
			local alpha = math.min(PlayerFrameDarkManaGlow:GetAlpha(), DARKMANAGLOW_MAXALPHA * (interval/(animTime*(animTime/2))));
			
			PlayerFrameDarkManaGlow:SetAlpha(alpha);
			if ( alpha <= DARKMANAGLOW_FADEALPHA ) then
				self.timeSinceThrob = 0;
				DARKMANAGLOW_THROBSTART = 0;
				PlayerFrameDarkManaGlow:Hide();
