...
export const BARBARIAN_CLASS = std.Classes.create(
  // Mod id
...
// Change Class Icon
BARBARIAN_CLASS.UI.setIcon(
  std.Image.readFromModule(
    "hardmerged-core",
    "/classmodules/assets/ClassIcons/barbarian-class-icon5.blp"
  )
);
...
BARBARIAN_CLASS.UI.Info.add(
  "- Recklessly dives into battle with little regard for personal safety"
);
BARBARIAN_CLASS.UI.Info.add("- Uses Rage to fuel their abilities");
BARBARIAN_CLASS.UI.Description.set(
  "Barbarians are fearless warriors who harness their primal fury to dominate in melee combat. With unmatched ferocity, they tear through enemies as berserkers or hunt their prey with deadly precision. Barbarians rely on raw strength and instinct to thrive on the battlefield, shunning magic for pure physical power."
);
...
//SkillLines
export const BRUTE_SKILL = std.SkillLines.create("hardmerged", "brute-skill");
BRUTE_SKILL.Name.enGB.set("Brute");

export const BERSERKER_SKILL = std.SkillLines.create(
  "hardmerged",
  "berserker-skill"
);
BERSERKER_SKILL.Name.enGB.set("Berserker");

export const SKIRMISHER_SKILL = std.SkillLines.create(
  "hardmerged",
  "skirmisher-skill"
);
