export function Main(events: TSEvents) {
    ...
    events.Spell.OnCast(UTAG("hardmerged", "spell.splitstep"), (spell) => {
    ...
    events.Spell.OnRemove(UTAG("hardmerged", "spell.splitstep"), (effect, application, type) => {

        const caster = ToPlayer(effect.GetCaster());
        if ( !caster ) return;
        
        caster.CastSpell(caster, UTAG("hardmerged", "spell.splitstep-second"), true);

    });



    events.Spell.OnCast(UTAG("hardmerged", "spell.splitstep-second"), (spell) => {
    ...
    events.Spell.OnRemove(UTAG("hardmerged", "spell.splitstep-second"), (effect, application, type) => {

        const caster = ToPlayer(effect.GetCaster());
        if ( !caster ) return;
        
        caster.CastSpell(caster, UTAG("hardmerged", "spell.splitstep-third"), true);

    });



    events.Spell.OnCast(UTAG("hardmerged", "spell.splitstep-third"), (spell) => {
